...
### Phase 21: Sonar fixes — cancel ownership transfer + shell `[[` conditionals
*   **`internal/cli/signal.go` go:S7059:** `cancelOnInterrupt` derived a context via `context.WithCancel` but passed ownership of `cancel` into a returned closure; Sonar's dataflow did not recognize closure-transfer and demanded a defer (which would wrongly cancel before return). Fix: extract the two-phase handler into `watchSigint(errOut io.Writer, sigCh chan os.Signal, cancel context.CancelFunc) func()`. Passing `cancel` as a function argument is the compliant ownership transfer; the goroutine and cleanup bodies move verbatim, `notifySigint()` stays inside `cancelOnInterrupt` so test overrides still work, and the `cancelOnInterrupt` signature is unchanged (5 callers in root.go unaffected).
*   **`test/integration/*.sh` shelldre S7688:** replace the single trailing `[ "$pass_count" = "$test_count" ]` with `[[ "$pass_count" = "$test_count" ]]` in all 7 scripts (arch, centos, debian, fedora, opensuse, rocky, ubuntu). All shebangs are `#!/bin/bash`, so the bash-only `[[ ]]` is safe; behavior for string equality is identical and it avoids word-splitting/glob hazards.
*   **Verification:** `bash -n test/integration/*.sh`, `task lint`, `task lint:sh`, `task test`, `task check`. No new unit tests — `signal_test.go` covers all four handler paths and stays green; script change is CI-integration territory.
*   **Docs:** No user-facing change → no README/docs edits beyond this entry.
### Phase 22: S7631 — untrusted checkout in integration workflows (githubactions:S7631)
*   **Goal:** Remove the `actions/checkout` `ref: ${{ github.event.release.tag_name }}` in all 7 `test-integration-*.yml` workflows (flagged as "Forked repository code should not be checked out in privileged workflow contexts"), and fix a latent bug where that field is always empty under `workflow_run`/`workflow_dispatch` payloads.
*   **Changes per file:** (1) job-level `if:` guard on `test` — allow `workflow_dispatch`, or `workflow_run` only when `conclusion == 'success'` AND `head_repository.full_name == github.repository`; (2) drop `with: ref:` from checkout (default-branch, trusted base-repo context); (3) resolve release tag at runtime: `TAG="${EVENT_TAG:-$(gh release view --json tagName -q .tagName)}"`, event tag passed via `env: EVENT_TAG` (no `${{ }}` interpolation into `run:` — avoids template injection).
*   **Why trust guard:** "Auto Tag & Release" triggers only on `push: [main]`, so no fork code flows today, but the guard makes the non-fork invariant machine-checked rather than incidental — the rule's remediation intent.
*   **Verification:** `actionlint` 0 issues on all 7; `python3 yaml.safe_load` valid ×7; grep asserts no `ref: ${{` remains. Real workflow behavior validated by GitHub Actions post-push.
*   **Docs:** No user-facing change → no README/docs edits beyond this entry.
