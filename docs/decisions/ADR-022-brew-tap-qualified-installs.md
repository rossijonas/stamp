...
Related: [ADR-021](ADR-021-sudo-prompting-and-preflight.md) (unrelated);
...
- **No pre-consent side effects:** for a qualified name, stamp skips cask
  detection (`brew info --cask`, which could load the tap) and the install
  dry-run preview (`brew install --dry-run`, which could auto-tap). The
  confirmation prompt stands alone.
...
- **Tests:** brew validator table, brew preview-skip, install/remove routing,
  and reinstall tracking are covered.
