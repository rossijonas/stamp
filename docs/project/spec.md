...
3. A `Noop` preview (e.g. package already up to date, or remove of an absent package) fails fast with `nothing to do` — no prompt. Brew tap-qualified refs skip the dry-run preview (ADR-022), so this fast path does not apply to them.
