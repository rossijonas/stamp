...
- `task test:integration` - Builds the binary, runs integration smoke tests in Docker containers (Ubuntu, Debian, Fedora, CentOS Stream 10, Rocky Linux 9, Arch Linux, openSUSE Tumbleweed). Requires Docker or Podman (`podman-docker` on Fedora). The suite is network-bound (package-manager metadata fetches) and can be slow on congested mirrors; the NOPASSWD sudo-preflight assertion is network-independent.
- Integration CI (`Integration Tests`, `.github/workflows/integration.yml`) - A single matrixed workflow runs the suite on all seven distros. It builds `stamp` from source and runs after the `CI` workflow succeeds on a pull request (testing the PR head SHA), after `Auto Tag & Release` succeeds on `main` (testing `main`), or on manual dispatch with a `ref` input to validate any branch, tag, or SHA before release.
