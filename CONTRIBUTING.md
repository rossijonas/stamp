...
- Integration CI (`Integration Tests`, `.github/workflows/integration.yml`) - A single matrixed workflow runs the suite on all seven distros. It builds `stamp` from source and runs after the `CI` workflow succeeds on a pull request (testing the PR head SHA) or on a push to `main` (testing `main`), or on manual dispatch with a `ref` input to validate any branch, tag, or SHA before release.
